...
class JetsonExporter(object):
    ...
    #GPU 사용량(%)만 노출하는 메트릭 함수
    def __gpu_usage(self):
        gpu_usage_gauge = GaugeMetricFamily(
            name="gpu_usage_percentage",
            documentation="GPU Usage Percentage from Jetson Stats",
            labels=["statistic", "nvidia_gpu"],
            unit="percent"
        )
        ...
        for gpu_name in gpu_names:
            gpu_usage_gauge.add_metric([gpu_name, "usage"], value=self.jetson.jtop_stats["gpu"][gpu_name]["status"]["load"])

        return gpu_usage_gauge
